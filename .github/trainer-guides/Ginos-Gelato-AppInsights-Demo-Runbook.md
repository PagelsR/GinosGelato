...
# 🎨 SLIDE PROMPTS (deck generation, not live demo)

## Profiler and Snapshot Debugger — why it's a slide, not a demo

Both tools have a collection delay that does not survive contact with a
conference schedule. But they are the honest "one level deeper" answer after
Demo 3 (the time was in SQL) and Demo 4 (here's the exception) — so they earn a
slide, not a blade.

---

## ⭐ Prompt A — PowerPoint Copilot (use this one)

In PowerPoint, open **Copilot** and paste this. Keep it as-is: PowerPoint Copilot
responds best to short, conversational prompts and will ignore long design specs.

```text
Add one slide titled "When the Dependency Isn't the Answer" with four short bullets:

- Application Insights Profiler — sampled call stacks from production, showing which method is burning the time, not just which dependency
- Snapshot Debugger — captures local variable values at the moment an exception was thrown, opened straight from the exception in the portal
- Both run in production on Azure App Service, with no debugger attached and no code change
- Performance tells you WHERE. Profiler tells you WHICH LINE. Snapshot Debugger tells you WHY.

Use short phrases, not sentences. No paragraphs.
```

### If it gives you too much text

Follow up in the same Copilot chat with:

```text
Shorten every bullet to eight words or fewer and make the last bullet the visual emphasis of the slide.
```

### Speaker notes — paste these manually

PowerPoint Copilot will not reliably generate speaker notes from a prompt. Drop
this into the notes pane yourself:

> "I'm not demoing these two live — both have a collection delay that doesn't
> survive a conference schedule. But this is the honest next step. Everything I
> showed you today stopped at the dependency boundary. When the slow code is
> yours, Profiler names the method. When the stack trace isn't enough, Snapshot
> Debugger gives you the variable values at the moment it blew up. Both run in
> production, in the same resource you've been watching all session."

---

## Prompt B — other slide generators (longer, more prescriptive)

Use this only if you are generating the deck outside PowerPoint. It specifies
layout and design constraints that PowerPoint Copilot ignores.

```text
Create a single presentation slide for a technical conference session titled
"AI-Powered Observability: Master Your App Performance with Azure Application Insights."

Slide title: "When the Dependency Isn't the Answer"

Context for the slide: earlier in the session we used Application Insights to
prove a slow request's time was spent inside an Azure SQL dependency, and we
caught a browser exception the API never saw. This slide covers the two tools
you reach for when the bottleneck is inside your own .NET code rather than in a
dependency, and when an exception's stack trace isn't enough to explain it.

Structure the slide as two labeled columns:

Column 1 — Application Insights Profiler
- Answers: "the time is in MY code, not in SQL. Which method?"
- Captures sampled call stacks with wall-clock time attributed per method
- Runs on a low-overhead schedule in production, not in a debugger
- Supported on Azure App Service, which is where this API is hosted

Column 2 — Snapshot Debugger
- Answers: "I have the stack trace, but what were the VALUES?"
- Captures local variables and parameters at the moment an exception was thrown
- Opens directly from the exception in the Application Insights portal
- Debug-quality state from production, without attaching a debugger

Add a single bottom banner line, visually emphasized:
"Performance blade tells you WHERE. Profiler tells you WHICH LINE. Snapshot Debugger tells you WHY."

Design requirements:
- Clean, dark-on-light, high contrast, readable from the back of a large room
- Minimum 24pt body text; no paragraphs, short phrases only
- Two-column layout with clear visual separation
- Include the Azure Monitor / Application Insights icon if available
- No clip art, no stock photos, no decorative gradients

Speaker notes to include with the slide:
"I'm not demoing these two live — both have a collection delay that doesn't
survive contact with a conference schedule. But this is the honest next step.
Everything I showed you today stopped at the dependency boundary. When the slow
code is yours, Profiler names the method. When the stack trace isn't enough,
Snapshot Debugger gives you the variable values at the moment it blew up. Both
run in production. Both are in the same resource you've been watching all session."
```
